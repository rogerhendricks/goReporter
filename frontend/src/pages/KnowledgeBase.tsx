...
export default function KnowledgeBase() {
  ...
  useEffect(() => {
    ...
    async function loadManifest() {
      ...
      try {
        const res = await fetch('/knowledge-base/index.json', { cache: 'no-store' })
        if (!res.ok) {
          throw new Error(`Failed to load knowledge base index (HTTP ${res.status})`)
        }
        ...
        if (!cancelled) {
          setManifestError(err instanceof Error ? err.message : 'Failed to load knowledge base index')
        }
      } finally {
        if (!cancelled) {
  ...
  const resolvedSelection = useMemo(() => {
    ...
    const section = manifest.sections.find(s => s.id === sectionId) ?? manifest.sections[0]
  ...
  useEffect(() => {
    ...
    if (urlSectionId !== resolvedSelection.section.id || urlArticleId !== resolvedSelection.article.id) {
      navigate(`/knowledge-base/${resolvedSelection.section.id}/${resolvedSelection.article.id}`, { replace: true })
    }
  ...
  useEffect(() => {
    ...
    async function loadMarkdown(path: string) {
      ...
      try {
        ...
        if (!cancelled) {
          setMarkdown('')
          setMarkdownError(err instanceof Error ? err.message : 'Failed to load article')
        }
      } finally {
        if (!cancelled) {
  ...
  return (
    <div className="container mx-auto">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[320px_1fr]">
                        ...
                        <div className="space-y-1">
                          {section.articles.map((article) => {
                            ...
                            return (
                              ...
                              >
                                <Link to={`/knowledge-base/${section.id}/${article.id}`}>{article.title}</Link>
                              </Button>
                            )
                          })}
                        </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle>{resolvedSelection?.article.title ?? 'Article'}</CardTitle>
          </CardHeader>
          <CardContent>
            {loadingMarkdown ? (
              <div className="text-sm text-muted-foreground">Loading…</div>
              ...
              <div className="prose prose-sm max-w-none dark:prose-invert">
                <ReactMarkdown
                  components={{
                    ...
                    h2: (props) => <h2 className="scroll-m-20 border-b pb-2 text-2xl font-semibold tracking-tight" {...props} />,
