...
import { Plus, Edit, Trash2, FileText, CheckCircle2, XCircle, Download, LinkIcon, Mail, Send  } from 'lucide-react'
...
export default function PatientReportList() {
  ...
  const latestImplantedDevice = () => {
    ...
    return [...list].sort(
      (a, b) => new Date(b.implantedAt).getTime() - new Date(a.implantedAt).getTime()
    )[0]
  }

  const buildDefaultEmail = (report: any) => {
    const dev = latestImplantedDevice()
    const patientName = `${currentPatient?.fname ?? ''} ${currentPatient?.lname ?? ''}`.trim()
                      ...
                      <div>
                        ...
                        <div className="max-h-40 overflow-y-auto space-y-2">
                          {currentPatient?.patientDoctors?.length ? (
                            currentPatient.patientDoctors.map((pd) => (
                              <button
                                ...
                                className={`w-full text-left p-2 rounded border hover:bg-accent transition ${
                                  emailState[report.id]?.doctorId === pd.doctorId ? 'bg-accent' : ''
                                }`}
                            ...
                            <div className="text-sm text-muted-foreground">No doctors assigned.</div>
                          )}
                        </div>
                      </div>
                        ...
                        return (
                          <>
                            <div className="space-y-1">
                              ...
                              <Input
                                value={draft.subject}
                                onChange={(e) => updateEmailDraft(report, { subject: e.target.value })}
                            ...
                            <div className="space-y-1">
                              ...
                              <Textarea
                                rows={6}
                                value={draft.body}
                                onChange={(e) => updateEmailDraft(report, { body: e.target.value })}
