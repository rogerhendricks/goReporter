...
export default function DeviceDetail() {
  ...
  const { currentDevice, loading, fetchDevice, deleteDevice } =
    useDeviceStore();
  ...
  useEffect(() => {
    const label =
      currentDevice?.name || (loading ? "Loading..." : "Device Details");
    setItems([
      { label: "Home", href: "/" },
      { label: "Devices", href: "/devices" },
      { label, current: true },
    ]);
  }, [currentDevice, loading, setItems]);

  const handleDelete = async () => {
    if (
      currentDevice &&
      window.confirm(`Are you sure you want to delete ${currentDevice.name}?`)
    ) {
    ...
    <div className="container mx-auto py-6">
      ...
      <Card>
        ...
        <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            <strong>Type:</strong> {currentDevice.type}
          </div>
          <div>
            <strong>MRI Safe:</strong>
            <Badge
              variant={currentDevice.isMri ? "default" : "secondary"}
              className="ml-2"
            >
