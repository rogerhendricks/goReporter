...
export default function LeadDetail() {
  ...
  useEffect(() => {
    const label =
      currentLead?.name || (loading ? "Loading..." : "Lead Details");
    setItems([
      { label: "Home", href: "/" },
      { label: "Leads", href: "/leads" },
      { label, current: true },
    ]);
  }, [currentLead, loading, setItems]);

  const handleDelete = async () => {
    if (
      currentLead &&
      window.confirm(`Are you sure you want to delete ${currentLead.name}?`)
    ) {
  ...
  if (!currentLead) {
    return (
      <div className="container mx-auto py-6 text-center">Lead not found.</div>
    );
  }
    ...
    <div className="container mx-auto py-6">
      ...
      <Card>
        ...
        <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            <strong>Connector:</strong> {currentLead.connector}
          </div>
          <div>
            <strong>MRI Safe:</strong>
            <Badge
              variant={currentLead.isMri ? "default" : "secondary"}
              className="ml-2"
            >
